...
class RandomCaptchaTypeSettingsForm extends ConfigFormBase {
  ...
  protected function getEditableConfigNames() {
    return array('random_captcha_type.settings');
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (count($enabled_types) < 2) {
      drupal_set_message(t('You need at least two CAPTCHA types (other than %random_captcha_type).', ['%random_captcha_type' => 'Random CAPTCHA type']), 'error');
    }
    $form = [];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (count(array_filter($form_state->getValue('random_captcha_type_enabled_types'))) < 2) {
      $form_state->setErrorByName('random_captcha_type_enabled_types', t('You should select at least two CAPTCHA types.'));
    }
