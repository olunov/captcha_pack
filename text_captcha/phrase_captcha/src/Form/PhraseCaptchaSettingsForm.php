...
class PhraseCaptchaSettingsForm extends ConfigFormBase {
  ...
  protected function getEditableConfigNames() {
    return array('phrase_captcha.settings');
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['phrase_captcha_additional_word_quantity'] = array(
      ...
      '#options' => array_combine(array(0, 1, 2, 3, 4, 5), array(0, 1, 2, 3, 4, 5)),
      '#required' => TRUE,
    );
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (count(array_filter($form_state->getValue('phrase_captcha_word_selection_challenges'))) < 1) {
      $form_state->setErrorByName('phrase_captcha_word_selection_challenges', t('You need to select at least one word selection criterium'));
    }
